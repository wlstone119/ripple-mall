package com.stone.ripple.crawler.story;
...
public class StoryCrawler implements CrawlerStrategyInterface {
	...
	public List<String> execute(ParamVo paramVo) {
		try {
			...
			logger.error(String.format("运行爬虫时出现异常： 参数【%s】", StringUtil.stringify(paramVo)), e);
		}
		return null;
	}
				...
				public List call() {
					...
					for (int page = start; page < complete; page++) {
						try {
							...
							logger.info(String
									.format("线程：【" + Thread.currentThread().getName() + "】已完成第-> page: 【%s】 页", page));
						} catch (Exception e) {
							logger.error(String.format(
									"线程：【" + Thread.currentThread().getName() + "】爬取第-> page: 【%s】 页时出现异常！", page), e);
						}
					}
