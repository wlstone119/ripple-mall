...
public class DubboProviderFilter implements Filter {
	...
	private static Logger logger = Logger.getLogger(DubboProviderFilter.class);
	...
	public Result invoke(Invoker<?> invoker, Invocation invocation) throws RpcException {
		...
		logger.info(String.format("调用开始：clientip=>%s，%s=>%s， 入参=>%s", clientIp, clzName, methodName, params));
		long startTime = System.currentTimeMillis();
		Result result = invoker.invoke(invocation);
		String rlt = jsonFilter(JSON.toJSONString(result));
		logger.info(String.format("调用结束：result=>%s, time:%s", rlt, System.currentTimeMillis() - startTime));
		return result;
	}
